            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-table.heading sortable wire:click="sortBy('created_at')"
                                     :direction="$sortField === 'created_at' ? $sorDirection : null ">Joining Date
                    </x-table.heading>
                    ...
                    @forelse($users as $user)
                        <x-table.row wire:loading.class.delay="opacity-50">
                            ...
                            <x-table.cell>
                                <div class="flex items-center text-sm">
                                    <div>
                                        <p class="text-xs text-gray-600 dark:text-gray-400"><span class="font-semibold">Phone: </span>{{ $user->phone ?? 'N/A' }}
                                        </p>
                                        <p class="text-xs text-gray-600 dark:text-gray-400"><span class="font-semibold">Mobile: </span>{{ $user->mobile ?? 'N/A'  }}
                                        </p>
                                    </div>
                                </div>
                            </x-table.cell>
                        ...
                        <span
                            ...
                            <x-table.cell>
                                <div class="flex items-center space-x-4 text-sm">
                                    ...
                                    <button
{{--                                        @click="openModal"--}}
wire:click="confirmUserDeletion({{ $user->id }})" wire:loading.attr="disabled"
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                             viewBox="0 0 20 20">
    ...
    <x-modal wire:model="confirmingUserDeletion">
        <div class="p-6">
            <div class="grid place-content-center">
                <div class="h-16 w-16 inline-grid place-content-center m-auto p-3 text-red-600 bg-red-200 rounded-full ">
            ...
            <div class="my-3">
                <p class="text-sm text-gray-500 text-center ">{{ __('Do you really want to delete your User?') }}
                </p>
            ...
            <div class="flex flex-col items-center justify-end px-6 py-3 -mx-6 -mb-4 space-y-4 sm:space-y-0 sm:space-x-6 sm:flex-row bg-gray-50 dark:bg-gray-800">
                <button class="w-full px-5 py-3 text-sm font-medium leading-5 text-white text-gray-700 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 sm:px-4 sm:py-2 sm:w-auto active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gra" wire:click="$set('confirmingUserDeletion', false)"
                        wire:loading.attr="disabled">
                    {{ __('Cancel') }}
                </button>
                <button wire:click="deleteUser({{ $confirmingUserDeletion  }})" class="w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-red"
                        wire:loading.attr="disabled">
                    {{ __('Delete') }}
                </button>
            </div>
