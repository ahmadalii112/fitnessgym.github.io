                        ...
                        <x-table.row wire:loading.class.delay="opacity-50">
                                ...
                                <div class="flex items-center text-sm">
                                    ...
                                    <div>
                                       ...
                                       {{-- <span
                                            ...
                                            text-{{ ($user->feeStructure->status = 'Paid' &&  \Carbon\Carbon::parse($user->feeStructure->issue_fee_date)->diffInDays($user->feeStructure->due_fee_date, false) ) ? 'green' : 'red' }}-700
                                            bg-{{ ($user->feeStructure->status = 'Paid' &&  \Carbon\Carbon::parse($user->feeStructure->issue_fee_date)->diffInDays($user->feeStructure->due_fee_date, false)) ? 'green' : 'red' }}-100
                                            dark:text-white dark:bg-{{ ($user->feeStructure->status = 'Paid' &&  \Carbon\Carbon::parse($user->feeStructure->issue_fee_date)->diffInDays($user->feeStructure->due_fee_date, false))  ? 'green' : 'red' }}-600">
                                            @if( \Carbon\Carbon::parse($user->feeStructure->issue_fee_date)->diffInDays($user->feeStructure->due_fee_date, false) > 0 && $user->feeStructure->status = 'Paid')
                                                Paid
                                            @else
                                                Unpaid
                                            @endif
                            ...
                            <x-table.cell>
                                <div class="flex items-center">
                                    <div>
                                        <p class="text-sm text-gray-600 dark:text-gray-400"><span class="font-semibold">Phone: </span>{{ $user->phone ?? 'N/A' }}
                                        </p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400"><span class="font-semibold">Mobile: </span>{{ $user->mobile ?? 'N/A'  }}
                                        </p>
                                    </div>
                                </div>
                            </x-table.cell>
